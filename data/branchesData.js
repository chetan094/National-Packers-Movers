export const branchesData = {
  states: {
    'jharkhand': {
      name: 'Jharkhand',
      title: 'Best Packers and Movers in Jharkhand | Trusted Since 1987',
      description: 'National Packers & Movers is Jharkhand\'s oldest & most trusted relocation service since 1987. Offices in Dhanbad (HQ), Ranchi, Bokaro, and Deoghar. 100% insured moves.',
      keywords: 'packers and movers jharkhand, home shifting services jharkhand, packers movers dhanbad, relocation ranchi, bokaro steel city transport',
      introTitle: 'Moving Jharkhand Safely Since 1987',
      introText: 'With over 38 years of logistics excellence, National Packers & Movers is the premier brand for shifting in Jharkhand. Founded by Debabrata Jhampaty, we started our journey here and have expanded to cover the entire state. From household shifting in Ranchi to heavy industrial transport in Dhanbad and Bokaro, we handle every relocation with our principle of absolute honesty.',
      cities: ['dhanbad', 'ranchi', 'bokaro', 'deoghar', 'hazaribagh'],
      stats: [
        { number: 45000, suffix: '+', label: 'Moves in Jharkhand' },
        { number: 38, suffix: '', label: 'Years Trusted Legacy' },
        { number: 6, suffix: '', label: 'Active Branches' },
        { number: 100, suffix: '%', label: 'Transit Safety' }
      ],
      testimonials: [
        { name: 'Rakesh Kumar Rathore', text: 'Excellent service. Shifting from Dhanbad to Pune was handled smoothly. The crew was quick and disciplined. Highly recommend them as the best packers and movers in Jharkhand.', rating: 5, initials: 'RR' },
        { name: 'Simran', text: 'I had a great experience shifting from Ranchi to Mumbai. Very fast, friendly, and completely damage-free. Definitely one of the top home shifting services in Jharkhand.', rating: 5, initials: 'S' }
      ]
    },
    'west-bengal': {
      name: 'West Bengal',
      title: 'Top Packers and Movers in West Bengal | Safe & Insured Shifting',
      description: 'Professional packing & shifting services in West Bengal. Secure relocations in Kolkata, Durgapur, and Asansol. IBA approved billing, secure transit.',
      keywords: 'packers and movers west bengal, kolkata shifting company, movers asansol, durgapur industrial shifting',
      introTitle: 'Your Reliable Relocation Partner in West Bengal',
      introText: 'West Bengal is a crucial hub in our Eastern India logistics network. National Packers & Movers offers high-end residential, corporate, and industrial relocation services. Whether it is navigating the busy lanes of Kolkata, moving families in the steel city of Asansol, or managing industrial logistics in Durgapur, our dedicated teams ensure a seamless transition.',
      cities: ['kolkata', 'durgapur', 'asansol'],
      stats: [
        { number: 35000, suffix: '+', label: 'Bengal Shifts' },
        { number: 3, suffix: '', label: 'Full Hubs' },
        { number: 100, suffix: '%', label: 'Claim Resolution' },
        { number: 24, suffix: 'Hr', label: 'Support Turnaround' }
      ],
      testimonials: [
        { name: 'Harman', text: 'Relocated from Kolkata to Chennai. Their customer service is exceptional, with a prompt response to all my queries. They are the best packers and movers in West Bengal.', rating: 5, initials: 'H' },
        { name: 'Arun', text: 'Meticulously handled my belongings during our shift from Asansol to Bhubaneswar. Highly professional packing standards and reliable packers movers in West Bengal.', rating: 5, initials: 'A' }
      ]
    },
    'bihar': {
      name: 'Bihar',
      title: 'Reliable Packers and Movers in Bihar | Home & Bike Transport',
      description: 'Experienced relocation experts in Bihar. Serving Patna and Bhagalpur. Safe door-to-door shifting with multi-layer packing. Get a free quote.',
      keywords: 'packers and movers bihar, packers movers patna, bhagalpur shifting company, bike relocation bihar',
      introTitle: 'Safe, Efficient, and Honest Shifting Across Bihar',
      introText: 'National Packers & Movers brings world-class packing and moving services to Bihar. We service the historic capital of Patna and the silk city of Bhagalpur, bridging relocations across Bihar to the rest of India. We utilize our own dedicated fleet of vehicles to eliminate third-party delays and damages.',
      cities: ['patna', 'bhagalpur'],
      stats: [
        { number: 30000, suffix: '+', label: 'Families Moved' },
        { number: 2, suffix: '', label: 'Key Cities' },
        { number: 100, suffix: '%', label: 'Insured Shipments' },
        { number: 8,   suffix: 'AM', label: 'to 8PM Customer Care' }
      ],
      testimonials: [
        { name: 'Aniket Kumar', text: 'Shifted from Patna to Delhi. Punctual, professional, and handled all household assets with utmost care. The best packers and movers in Bihar, zero hassle.', rating: 5, initials: 'AK' },
        { name: 'Pramjit Singh', text: 'Shifted from Nabinagar to Jhajjar. Very friendly staff and smooth transit coordination. Completely pain-free shifting. Exceptional movers in Bihar.', rating: 5, initials: 'PS' }
      ]
    },
    'madhya-pradesh': {
      name: 'Madhya Pradesh',
      title: 'Packers and Movers in Madhya Pradesh | Singrauli Relocations',
      description: 'Trusted movers and packers in Madhya Pradesh. Dedicated branch in Singrauli servicing PSU & mining corporate employee relocations.',
      keywords: 'packers and movers madhya pradesh, packers movers singrauli, coal mine relocation mp',
      introTitle: 'Logistics Leadership in Madhya Pradesh',
      introText: 'We support Madhya Pradesh\'s industrial relocation needs, with a specialized branch in Singrauli. We have handled hundreds of relocations for employees of NCL, NTPC, and other major corporations, offering professional paperwork, IBA-compliant billing, and heavy item handling.',
      cities: ['singrauli'],
      stats: [
        { number: 12000, suffix: '+', label: 'PSU Relocations' },
        { number: 1,    suffix: '', label: 'Industrial Hub' },
        { number: 100,  suffix: '%', label: 'IBA Billing Support' },
        { number: 0,    suffix: '%', label: 'Damage Rate' }
      ],
      testimonials: [
        { name: 'Mukesh', text: 'Relocated my household goods from Singrauli to Delhi. Provided closed container vehicle at appealing rates with premium packing. Highly recommend them as the best packers and movers in Madhya Pradesh.', rating: 5, initials: 'M' }
      ]
    },
    'odisha': {
      name: 'Odisha',
      title: 'Packers and Movers in Odisha | Virtual Office & Transport',
      description: 'Coordinate relocations in Bhubaneswar, Cuttack, and all Odisha cities through our virtual office network. Secure packing and full transit insurance.',
      keywords: 'packers and movers odisha, movers in bhubaneswar, cuttack relocation',
      introTitle: 'Seamless Logistics Coordination in Odisha',
      introText: 'National Packers & Movers services Odisha through a highly coordinated Virtual Office network. While all bookings and administration are run through our regional administrative coordination network, we dispatch specialized teams and vehicles from our neighboring hubs to execute safe, prompt relocations in Bhubaneswar, Cuttack, Rourkela, and beyond.',
      cities: ['virtual-office'],
      stats: [
        { number: 3500,  suffix: '+', label: 'Odisha Bookings' },
        { number: 1,    suffix: '', label: 'Central HQ Control' },
        { number: 100,  suffix: '%', label: 'Insured Goods' },
        { number: 7,    suffix: 'Days', label: 'Active Service' }
      ],
      testimonials: [
        { name: 'Jyoti Ranjan', text: 'My household movement from Bhubaneswar to Dhanbad was handled smoothly. Highly professional coordination by this Packers and Movers Odisha virtual office.', rating: 5, initials: 'JR' }
      ]
    },
    'uttar-pradesh': {
      name: 'Uttar Pradesh',
      title: 'Packers and Movers in Uttar Pradesh | Coming Soon',
      description: 'Expanding soon to major cities in Uttar Pradesh. Contact our HQ for current inter-state bookings to Lucknow, Noida, Kanpur, and Varanasi.',
      keywords: 'packers and movers up, packers movers lucknow, kanpur relocation',
      introTitle: 'Expanding Our Logistics Empire to Uttar Pradesh',
      introText: 'As part of our commitment to scale National Packers & Movers into a nationwide logistics powerhouse, we are establishing physical branch offices in Uttar Pradesh soon. Currently, we manage daily inter-state relocation trucks running from Jharkhand/Bengal to major UP cities including Lucknow, Kanpur, Varanasi, and Noida.',
      cities: ['coming-soon'],
      stats: [
        { number: 90000, suffix: '+', label: 'Cross-State Moves' },
        { number: 5,    suffix: '+', label: 'Daily Trucks to UP' },
        { number: 38,   suffix: '+', label: 'Years Legacy' },
        { number: 100,  suffix: '%', label: 'Assured Delivery' }
      ],
      testimonials: [
        { name: 'R. P. Mishra', text: 'I booked my household shift from Dhanbad to Varanasi. The team delivered everything in 3 days. The packing of our fragile kitchen items was robust. Excellent packers and movers in Uttar Pradesh.', rating: 5, initials: 'RPM' }
      ]
    }
  },
  cities: {
    'dhanbad': {
      name: 'Dhanbad (HQ)',
      stateSlug: 'jharkhand',
      stateName: 'Jharkhand',
      latitude: 23.8014,
      longitude: 86.4299,
      title: 'Packers and Movers in Dhanbad (HQ) | National Packers & Movers',
      description: 'HQ of National Packers & Movers. 38+ years of trusted home shifting, office relocation, and vehicle transport in Dhanbad. Call 9835168368 for free quote.',
      keywords: 'packers and movers dhanbad, best packers movers dhanbad, house shifting dhanbad, office relocation dhanbad, trusted packers 1987',
      address: 'Shop No. 1, Building Campus, Kutchery Road, beside Green View Petrol Pump, Kasturba Nagar, Dhanbad, Jharkhand — 826001',
      phone: '9835168368 / 9934166164',
      justdial: 'https://jsdl.in/DT-99I3Q9LVTWT',
      justdialRating: 4.7,
      justdialReviewCount: 490,
      mapEmbed: 'https://maps.google.com/maps?q=National%20Packers%20%26%20Movers%2C%20Beside%20Green%20View%20Petrol%20Pump%2C%20Kasturba%20Nagar%2C%20Dhanbad%2C%20Jharkhand%20826001&t=&z=15&ie=UTF8&iwloc=&output=embed',
      tagline: 'Our Headquarters — The Core of Our Operations',
      introText: 'As our central headquarters, our Dhanbad branch represents the pinnacle of National Packers & Movers. Since 1987, we have been the undisputed leaders in home shifting, office relocation, and industrial transport in Dhanbad, Katras, Jharia, and surrounding coalfields. With our fully owned fleet, background-verified packing crew, and direct supervisor monitoring, your move is in safe, professional hands.',
      faqs: [
        { q: 'Where is your main office located in Dhanbad?', a: 'Our central headquarters is located at Shop No. 1, Building Campus, Kutchery Road, beside Green View Petrol Pump, Kasturba Nagar, Dhanbad, Jharkhand - 826001. You are welcome to visit us, or call us at 9835168368 to schedule a free pre-move survey.' },
        { q: 'Do you offer local packing and shifting within Dhanbad city?', a: 'Yes, we handle local home and office shifting within Dhanbad (Jharia, Katras, Govindpur, Saraidhela) as well as long-distance moves from Dhanbad to any city across India.' },
        { q: 'Are your packing charges transparent?', a: 'Absolutely. We stand by our motto: "Honesty is not our policy, but our Principle." The quote we give you after assessing your goods is final. There are zero hidden costs or surprise surcharges.' }
      ],
      testimonials: [
        { name: 'Rakesh Kumar Rathore', text: 'My experience with National Packers & Movers was excellent. They shifted my household items from Dhanbad to Pune without a scratch. Highly professional crew. Location is central in Kasturba Nagar, Dhanbad, making them the best packers and movers in Dhanbad.', rating: 5, initials: 'RR' },
        { name: 'SHORYA PRATAP', text: 'I recently used National Packers & Movers for my relocation from Dhanbad to Delhi, and I couldn\'t be more satisfied! Their safe packaging ensured that all my belongings arrived in perfect condition. Best packers movers Dhanbad.', rating: 5, initials: 'SP' },
        { name: 'Amir Sohel', text: 'Awesome service shifting within Dhanbad. They took care of my goods as their own. Their workforce is very skilled and behave like my son (very polite). Best house shifting Dhanbad.', rating: 5, initials: 'AS' }
      ]
    },
    'ranchi': {
      name: 'Ranchi',
      stateSlug: 'jharkhand',
      stateName: 'Jharkhand',
      latitude: 23.3441,
      longitude: 85.3096,
      title: 'Packers and Movers in Ranchi | Household Shifting Services',
      description: 'Professional household, corporate & vehicle relocation in Ranchi. 100% insured, multi-layer packing, transparent pricing. Contact Ranchi branch coordinators.',
      keywords: 'packers and movers ranchi, home shifting ranchi, office shifting ranchi, best packers movers ranchi',
      address: 'Ranchi Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'Leading Shifting Experts in the Capital City',
      introText: 'Our Ranchi branch extends our 38-year legacy of trust to the capital city of Jharkhand. We cater to the residential moving needs of Ranchi\'s growing population, offering customized services in Lalpur, Kanke, Bariatu, Morabadi, and Hinoo. From high-quality bubble wrapping to heavy lifting of wooden furniture and electronics, we ensure a stress-free move.',
      faqs: [
        { q: 'Do you offer vehicle (car/bike) transport from Ranchi?', a: 'Yes, we relocate cars and two-wheelers from Ranchi to anywhere in India using specialized, closed car carriers.' },
        { q: 'How long does a household shift from Ranchi to Kolkata take?', a: 'A standard household move from Ranchi to Kolkata typically takes 2 to 3 days, including packing, loading, transit, and unloading.' }
      ],
      testimonials: [
        { name: 'Simran', text: 'I had a great experience with National Packers & Movers during my home shift from Ranchi to Mumbai. Their team was very quick and helped me move my things without any delay. They were friendly and worked fast. Best packers and movers in Ranchi.', rating: 5, initials: 'S' },
        { name: 'Raju Upadhyay', text: 'Their service is very good, I got all my household stuff shifted from Ranchi to Patna. Brought it very properly and safely. Best home shifting Ranchi.', rating: 5, initials: 'RU' }
      ]
    },
    'bokaro': {
      name: 'Bokaro',
      stateSlug: 'jharkhand',
      stateName: 'Jharkhand',
      latitude: 23.659194,
      longitude: 86.152283,
      title: 'Packers and Movers in Bokaro Steel City | Employee Relocations',
      description: 'Reliable shifting services in Bokaro Steel City. Dedicated packing & moving services for BSL employees, fully insured transit. Get a free quote.',
      keywords: 'packers and movers bokaro, packers movers bokaro steel city, home shifting bokaro, bsl employee relocation',
      address: 'Indira Gandhi Marg, Sector 3/C, Near Bokaro Mall, Bokaro Steel City, Sector 3, Bokaro-827003, Jharkhand',
      phone: '9835168368 / 9934166164',
      justdial: 'https://jsdl.in/DT-99RKM1VIUJ9',
      justdialRating: 5.0,
      justdialReviewCount: 160,
      mapEmbed: 'https://maps.google.com/maps?q=23.659194,86.152283(National%20Packers%20%26%20Movers%20-%20Bokaro)&t=&z=15&ie=UTF8&iwloc=&output=embed',
      tagline: 'Reliable Moving Partners for the Steel City',
      introText: 'Bokaro Steel City is a major hub of public sector employees. We specialize in handling employee relocations for Bokaro Steel Plant (BSL) and other corporate entities. We understand the reimbursement requirements of PSU employees and provide complete, verified billing documents, itemized packing lists, and IBA-aligned transport receipts.',
      faqs: [
        { q: 'Do you provide valid invoices for PSU relocation reimbursement in Bokaro?', a: 'Yes, we provide full, GST-compliant tax invoices, itemized loading sheets, and transport bills that are fully accepted for reimbursement claims.' },
        { q: 'Is transit insurance available for moves out of Bokaro?', a: 'Yes, comprehensive transit insurance is mandatory for all our inter-city relocations out of Bokaro to guarantee full safety.' }
      ],
      testimonials: [
        { name: 'Kirpal Kumar', text: 'I recently used National Packers & Movers for my relocation from Bokaro to Kolkata. They provided a systematic approach to moving, ensuring everything was organized and efficient. Highly professional packers and movers in Bokaro Steel City.', rating: 5, initials: 'KK' },
        { name: 'Amit Dayal', text: 'They are very professional and shifted my household goods from Bokaro Steel City to Ranchi safely. High-quality bubble wraps and heavy-duty box packing were used. Commendable home shifting Bokaro.', rating: 5, initials: 'AD' },
        { name: 'Harman', text: 'I had a good experience with National Packers and Movers. Their prices are reasonable and the instructions they gave me were clear and helpful. I felt supported during the moving process. Overall, I am happy with their service.', rating: 5, source: 'justdial' },
        { name: 'Ankit kumar', text: 'National Packers and Movers is excellent! The location is central, making it simple for everyone to reach. The staff is friendly and helpful. They make moving fun and stress-free. I had a great experience using their services.', rating: 5, source: 'justdial' },
        { name: 'Aman kumar', text: 'I recently used National Packers and Movers for my relocation, and I couldn\'t be more satisfied! From packing to unloading, they handled my belongings with utmost care. Highly recommend their services for a stress-free move!', rating: 5, source: 'justdial' },
        { name: 'Aniket kumar', text: 'I recently used National Packers and Movers, and I am very impressed! Their quick assistance was excellent. The team was friendly and worked efficiently, making my experience stress-free. I highly recommend them!', rating: 5, source: 'justdial' },
        { name: 'Mohit Sheladiya', text: 'National Packers and Movers is great! When I needed to move, they came quickly and were very friendly. They packed my things safely and took care of everything. Excellent job, recommend them!', rating: 5, source: 'justdial' }
      ]
    },
    'deoghar': {
      name: 'Deoghar',
      stateSlug: 'jharkhand',
      stateName: 'Jharkhand',
      latitude: 24.491825,
      longitude: 86.694692,
      title: 'Packers and Movers in Deoghar | Safe Home Shifting',
      description: 'Secure household relocation, luggage shifting & bike transport in Deoghar. 100% safe packing, verified staff. Contact us today.',
      keywords: 'packers and movers deoghar, house shifting deoghar, luggage transport deoghar',
      address: 'Tower Chowk, Branch Office Dhanbad, Main Road, Deoghar, Deoghar-Jharkhand-814112, Jharkhand',
      phone: '9835168368 / 9934166164',
      justdial: 'https://jsdl.in/DT-997GWBZ7QFB',
      justdialRating: 5.0,
      justdialReviewCount: 290,
      mapEmbed: 'https://maps.google.com/maps?q=24.491825,86.694692(National%20Packers%20%26%20Movers%20-%20Deoghar)&t=&z=15&ie=UTF8&iwloc=&output=embed',
      tagline: 'Serving the Holy City with Devotion and Care',
      introText: 'We bring our professional moving standards to Deoghar, ensuring that your households, luggage, and commercial goods are relocated with the highest standards of safety, care, and honesty. Whether you are moving to/from Jasidih, Madhupur, or Deoghar town, our packers handle your belongings with reverence and care.',
      faqs: [
        { q: 'Can I book bike transportation from Deoghar?', a: 'Yes, we pack bikes with multiple layers of bubble wrap, corrugated sheets, and stretch film to prevent any scratches during transit.' }
      ],
      testimonials: [
        { name: 'S. R. Verma', text: 'Very genuine service. All my goods are well transported from Deoghar to Noida. I will proudly recommend National Packers and Movers as the best movers and packers in Deoghar. Thanks!', rating: 5, initials: 'SRV' },
        { name: 'Rakesh Kumar Rathore', text: 'I had an excellent experience with National Packers And Movers. They offer safe packaging, which made me feel confident about my items. The prices are reasonable and their service is great. Highly recommend!', rating: 5, source: 'justdial' },
        { name: 'SHORYA PRATAP', text: 'I recently used National Packers And Movers, and I am very happy with their service! They gave clear instructions throughout, which made everything easy. My items were carefully handled and packed safely.', rating: 5, source: 'justdial' },
        { name: 'Raviraj', text: 'I recently used National Packers And Movers for my move, and I was really impressed! Their service was very systematic, making the whole process smooth. Their prices were reasonable and affordable.', rating: 5, source: 'justdial' },
        { name: 'arun', text: 'I had a good experience with National Packers And Movers. Their prices are reasonable. The team was careful with my things. They handled all my items gently and made sure nothing got damaged.', rating: 5, source: 'justdial' },
        { name: 'Vaibhav Sharma', text: 'I had a great experience with National Packers and Movers! They helped me quickly when I needed to move. The team gave clear instructions, so I knew what to do. My things arrived safely and on time.', rating: 5, source: 'justdial' }
      ]
    },
    'hazaribagh': {
      name: 'Hazaribagh',
      stateSlug: 'jharkhand',
      stateName: 'Jharkhand',
      latitude: 24.006557,
      longitude: 85.348945,
      title: 'Packers and Movers in Hazaribagh | Safe Relocation Services',
      description: 'Reliable Packers and Movers in Hazaribagh. 38+ years of trust, local & domestic shifting, IBA-aligned billing, 100% safety insured. Call 9835168368.',
      keywords: 'packers and movers hazaribagh, home shifting hazaribagh, transport hazaribagh',
      address: 'Matwari Chowk, Main Road, Hazaribagh Katkamsandi Road, Hazaribagh-825301, Jharkhand',
      phone: '9835168368 / 9934166164',
      justdial: 'https://jsdl.in/DT-992JMRKVWI1',
      justdialRating: 5.0,
      justdialReviewCount: 120,
      mapEmbed: 'https://maps.google.com/maps?q=24.006557,85.348945(National%20Packers%20%26%20Movers%20-%20Hazaribagh)&t=&z=15&ie=UTF8&iwloc=&output=embed',
      tagline: 'Trusted Moving Experts in Hazaribagh',
      introText: 'Our Hazaribagh branch brings professional packers and movers services to the residents of Hazaribagh. With multiple layers of safe packaging, verified loaders, and dedicated container trucks, we coordinate home shifting, office relocation, and bike transportation with complete transparency and zero hidden costs.',
      faqs: [
        { q: 'Where is your Hazaribagh office located?', a: 'Our office is located at Matwari Chowk, Hazaribagh Katkamsandi Road, Hazaribagh-825301, Jharkhand. You can reach us at 9835168368.' },
        { q: 'Do you offer transit insurance for shifting from Hazaribagh?', a: 'Yes, we provide full transit insurance for all moves originating from Hazaribagh to ensure your peace of mind.' }
      ],
      testimonials: [
        { name: 'Raviraj', text: 'I had a great experience with National Packers and Movers! They were very easy to reach, making the whole process smooth. The team was friendly and professional. I also found their prices to be reasonable.', rating: 5, source: 'justdial' },
        { name: 'Kirpal Kumar', text: 'My experience with National Packers and Movers was exceptional. Their easily accessible services made scheduling a breeze. The safety of my belongings was clearly a priority, as they used high-quality packaging.', rating: 5, source: 'justdial' },
        { name: 'Ankit kumar', text: 'I recently used National Packers and Movers for my relocation, and I couldn\'t be happier! Their customer service team was exceptional—responsive, friendly, and always willing to assist.', rating: 5, source: 'justdial' },
        { name: 'Aman kumar', text: 'My experience with National Packers and Movers was excellent! They handled my belongings with utmost care and professionalism. Every item was meticulously wrapped and secured.', rating: 5, source: 'justdial' },
        { name: 'Aniket kumar', text: 'My experience with National Packers and Movers was exceptional! They handled my belongings with utmost care, ensuring everything arrived in perfect condition. Highly recommend!', rating: 5, source: 'justdial' }
      ]
    },
    'kolkata': {
      name: 'Kolkata',
      stateSlug: 'west-bengal',
      stateName: 'West Bengal',
      latitude: 22.5726,
      longitude: 88.3639,
      title: 'Packers and Movers in Kolkata | Professional Home Shifting',
      description: 'Top-tier packers and movers in Kolkata. Residential shifting, corporate office moves, storage solutions, and vehicle transport. Fast, insured service.',
      keywords: 'packers and movers kolkata, best packers movers kolkata, house shifting kolkata, office shifting kolkata',
      address: 'Kolkata Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'Connecting the City of Joy to All India',
      introText: 'Kolkata is a vital metropolitan pivot in our operations. We specialize in seamless relocations into and out of Kolkata. Our packers are trained to handle high-rise apartment shifting, navigating narrow streets with feeder vans, and coordinating heavy container movements on long-distance routes to other states.',
      faqs: [
        { q: 'How do you handle moving in narrow streets of North/South Kolkata?', a: 'We employ smaller feeder trucks to load your goods from narrow streets, transfer them securely to our large carrier container at our consolidation point, and then begin the highway transit.' },
        { q: 'Do you offer packing services on Sundays in Kolkata?', a: 'Yes, we operate 7 days a week, from 8:00 AM to 8:00 PM, to suit your schedule.' }
      ],
      testimonials: [
        { name: 'Harman', text: 'I recently used National Packers & Movers for my relocation from Salt Lake, Kolkata to Chennai, and I couldn\'t be happier! Exceptional response and stress-free transit. Best packers and movers in Kolkata.', rating: 5, initials: 'H' },
        { name: 'Laltu Bose', text: 'Best packers and movers with seamless relocation from Kolkata to Jamshedpur. They provided me with a very reasonable rate, and offered premium and customized packing. Highly recommend for Kolkata shifting.', rating: 5, initials: 'LB' }
      ]
    },
    'durgapur': {
      name: 'Durgapur',
      stateSlug: 'west-bengal',
      stateName: 'West Bengal',
      latitude: 23.4808,
      longitude: 87.3190,
      title: 'Packers and Movers in Durgapur | Industrial & Home Shifting',
      description: 'Expert relocation services in Durgapur. Secure household packing, bike transport, and specialized heavy machinery transport in the steel hub.',
      keywords: 'packers and movers durgapur, home shifting durgapur, industrial transport durgapur',
      address: 'Durgapur Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'Logistics Experts in the Industrial Hub',
      introText: 'Durgapur\'s industrial landscape demands heavy logistics capability. We specialize in residential relocations for corporate executives, plant managers, and PSU employees, alongside transporting machinery, warehouse goods, and industrial tools with absolute reliability.',
      faqs: [
        { q: 'Can you transport heavy machinery or factory goods in Durgapur?', a: 'Yes, we have specialized flatbeds, trucks, and specialized equipment to handle industrial cargo relocations.' }
      ],
      testimonials: [
        { name: 'Aman Kumar', text: 'My experience with National Packers & Movers was outstanding during our shift from Durgapur to Patna! Their attention to safe packaging was remarkable. Every item was meticulously wrapped and secured. Best packers and movers in Durgapur.', rating: 5, initials: 'AK' },
        { name: 'Gora Chand Bose', text: 'I had a wonderful experience shifting from Durgapur to Haldia. Everyone is very courteous and understanding. Hassle-free service. Excellent Durgapur industrial shifting.', rating: 5, initials: 'GB' }
      ]
    },
    'asansol': {
      name: 'Asansol',
      stateSlug: 'west-bengal',
      stateName: 'West Bengal',
      latitude: 23.6740,
      longitude: 86.9730,
      title: 'Packers and Movers in Asansol | Premium Relocation Services',
      description: 'Reliable local & interstate house shifting in Asansol. High-quality packing materials, specialized vehicle shifting. Inquire for a quote.',
      keywords: 'packers and movers asansol, home shifting asansol, packers movers asansol west bengal',
      address: 'Asansol Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'Your Shifting Partner in the Coal-and-Industrial Belt',
      introText: 'Serving the key industrial hub of Asansol, National Packers & Movers delivers unmatched quality in packing. We use heavy-duty cardboard boxes, high-density bubble wrap, and stretch film to shield your furniture and belongings from interstate highway vibration.',
      faqs: [
        { q: 'Do you offer daily shifting service from Asansol to Dhanbad?', a: 'Yes, this is one of our most active interstate routes. We offer next-day delivery for moves between Asansol and Dhanbad.' }
      ],
      testimonials: [
        { name: 'Arun', text: 'My experience with National Packers & Movers was exceptional during my move from Asansol to Bhubaneswar. They meticulously handled my belongings with great care. Highly recommend them as the best packers and movers in Asansol.', rating: 5, initials: 'A' }
      ]
    },
    'patna': {
      name: 'Patna',
      stateSlug: 'bihar',
      stateName: 'Bihar',
      latitude: 25.5941,
      longitude: 85.1376,
      title: 'Packers and Movers in Patna | House & Commercial Shifting',
      description: 'Trusted household shifting and commercial relocation in Patna. Verified packing crew, transparent pricing, IBA-friendly billing. Call today.',
      keywords: 'packers and movers patna, house shifting patna, packers movers patna bihar, office shifting patna',
      address: 'Patna Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'Relocating Patna with Honor and Care',
      introText: 'We bring our 38 years of packing mastery to the historic city of Patna. From local relocations in Kankarbagh, Boring Road, and Bailey Road to long-distance moves to Delhi, Ranchi, or Kolkata, our Patna coordination team handles your relocation smoothly with 100% itemized inventory control.',
      faqs: [
        { q: 'What is the cost of shifting a 2BHK flat within Patna?', a: 'Local shifting costs vary based on the floor, lift availability, and volume of items. We provide a quick phone estimate followed by a free site survey.' }
      ],
      testimonials: [
        { name: 'Aniket Kumar', text: 'I recently used National Packers & Movers for my relocation from Patna to Delhi, and I couldn\'t be more impressed! Their team was punctual, professional, and handled my belongings with utmost care. Best packers and movers in Patna.', rating: 5, initials: 'AK' },
        { name: 'Sanjay K. Choudhary', text: 'Goods delivered safely from Patna to Bangalore. Professional service, careful handling, and polite staff. Trustworthy company. Overall, very good experience. Highly recommended for house shifting Patna.', rating: 5, initials: 'SKC' }
      ]
    },
    'bhagalpur': {
      name: 'Bhagalpur',
      stateSlug: 'bihar',
      stateName: 'Bihar',
      latitude: 25.2425,
      longitude: 87.0145,
      title: 'Packers and Movers in Bhagalpur | Budget-Friendly Relocations',
      description: 'Secure household relocation & vehicle transport in Bhagalpur. Background-verified team, budget-friendly rates, full insurance.',
      keywords: 'packers and movers bhagalpur, home shifting bhagalpur, local packers bhagalpur',
      address: 'Bhagalpur Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'Your Shifting Guides in the Silk City',
      introText: 'Bhagalpur residents can now leverage the nationwide network of National Packers & Movers. We provide premium packing services, bike logistics, and secure warehousing options. Our team is fully trained to wrap delicate silks, glass items, and heavy wooden furniture securely.',
      faqs: [
        { q: 'Do you pack glass and delicate kitchenware in Bhagalpur?', a: 'Yes, we use special box partitions, newspapers, and bubble sheets to wrap and pack kitchen glassware securely.' }
      ],
      testimonials: [
        { name: 'Ritesh Kumar', text: 'I had a great experience moving from Bhagalpur to Patna. They gave clear and proper instructions for my move. Friendly team. Everything arrived on time. Best packers and movers in Bhagalpur.', rating: 5, initials: 'RK' }
      ]
    },
    'singrauli': {
      name: 'Singrauli',
      stateSlug: 'madhya-pradesh',
      stateName: 'Madhya Pradesh',
      latitude: 24.2001,
      longitude: 82.6841,
      title: 'Packers and Movers in Singrauli | PSU Shifting Specialists',
      description: 'Specialized household relocation in Singrauli for NCL, NTPC & power plant employees. Complete reimbursement documentation, IBA alignment.',
      keywords: 'packers and movers singrauli, packers movers singrauli mp, ncl employee relocation, home shifting singrauli',
      address: 'Singrauli Branch Office (Bookings coordinated through HQ)',
      phone: '9835168368 / 9934166164',
      tagline: 'The Industrial Powerhouse Relocation Partners',
      introText: 'Singrauli is the energy hub of India, hosting large populations of PSU and power plant employees. National Packers & Movers has become the go-to specialist for employee shifting in Singrauli, Waidhan, and Vindhyanagar. We provide standard formats for reimbursement bills, inventory certificates, and transport receipts aligned with corporate guidelines.',
      faqs: [
        { q: 'Do you provide support for NCL/NTPC employee shifting reimbursement?', a: 'Yes. We provide verified GST invoices, money receipts, and packing lists in formats required by NCL, NTPC, and other PSUs for hassle-free claim approval.' },
        { q: 'Can you relocate our household items to Jharkhand or UP from Singrauli?', a: 'Yes, we run regular transport routes from Singrauli to Jharkhand, Bihar, UP, and West Bengal weekly.' }
      ],
      testimonials: [
        { name: 'Vinod Ram', text: 'Amazing service by National Packers and Movers. They provided fast loading, safe packing and transit for me. I shifted from Dhanbad to Singrauli. I strongly recommend this packers for safe relocation. Best packers and movers in Singrauli.', rating: 5, initials: 'VR' },
        { name: 'Mukesh', text: 'Best packers and movers for household relocation service. I strongly recommend National Packers & Movers. They relocated my goods from Singrauli to Delhi with a closed container truck at a very appealing cost and gave a premium service. Best packers movers Singrauli.', rating: 5, initials: 'M' }
      ]
    },
    'coming-soon': {
      name: 'Uttar Pradesh (Coming Soon)',
      stateSlug: 'uttar-pradesh',
      stateName: 'Uttar Pradesh',
      title: 'Packers and Movers in Uttar Pradesh | Coming Soon',
      description: 'Expanding soon to major cities in Uttar Pradesh. Reach our HQ for current cross-border transport and vehicle carrier bookings to UP.',
      keywords: 'packers and movers up, packers movers lucknow, kanpur relocation, varanasi packers',
      address: 'Upcoming Offices in UP (Contact HQ for current inter-state bookings)',
      phone: '9835168368 / 9934166164',
      tagline: 'Expanding Our Logistics Empire to Uttar Pradesh',
      introText: 'To fulfill Chetan Jhampaty\'s vision of growing National Packers & Movers into a national logistics leader, we are preparing to launch physical branch offices in Uttar Pradesh soon. Currently, we manage daily inter-state relocation trucks running from Jharkhand and West Bengal to major UP cities including Lucknow, Kanpur, Varanasi, Allahabad, and Noida.',
      faqs: [
        { q: 'Can I book a move from Dhanbad to Lucknow now?', a: 'Yes, we handle inter-state shifting from Jharkhand/Bengal to all cities in Uttar Pradesh daily.' }
      ],
      testimonials: [
        { name: 'R. P. Mishra', text: 'I booked my household shift from Dhanbad to Varanasi. The team delivered everything in 3 days. The packing of our fragile kitchen items was robust. Excellent service.', rating: 5, initials: 'RPM' }
      ]
    }
  }
};

export const STATE_CITIES = {
  'jharkhand': [
    'dhanbad', 'ranchi', 'bokaro', 'deoghar', 'jamshedpur', 'hazaribagh', 'giridih', 
    'ramgarh', 'medininagar', 'daltonganj', 'chas', 'adityapur', 'dumka', 'chatra', 
    'gumla', 'kodarma', 'koderma', 'pakur', 'sahibganj', 'sahebganj', 'simdega', 
    'latehar', 'khunti', 'saraikela', 'garhwa', 'lohardaga', 'ghatsila', 'phusro', 
    'katras', 'jharia', 'govindpur', 'dhansar', 'chirkunda', 'sindri', 'jasidih', 'madhupur'
  ],
  'west-bengal': [
    'kolkata', 'durgapur', 'asansol', 'siliguri', 'howrah', 'darjeeling', 'kharagpur', 
    'haldia', 'bardhaman', 'burdwan', 'malda', 'jalpaiguri', 'cooch-behar', 'purulia', 
    'bankura', 'midnapore', 'medinipur', 'krishnanagar', 'barasat', 'barrackpore', 
    'serampore', 'chinsurah', 'shantiniketan', 'bolpur', 'raniganj', 'burnpur', 'salt-lake', 'newtown', 'rajarhat'
  ],
  'bihar': [
    'patna', 'bhagalpur', 'gaya', 'muzaffarpur', 'purnia', 'darbhanga', 'bihar-sharif', 
    'ara', 'arrah', 'begusarai', 'katihar', 'munger', 'chhapra', 'danapur', 'bettiah', 
    'saharsa', 'hajipur', 'sasaram', 'motihari', 'siwan', 'madhubani', 'buxar', 'jehanabad', 
    'aurangabad', 'nawada', 'jamui', 'kishanganj', 'samastipur', 'lakhisarai', 'gopalganj'
  ],
  'madhya-pradesh': [
    'singrauli', 'waidhan', 'bhopal', 'indore', 'jabalpur', 'gwalior', 'ujjain', 'sagar', 
    'dewas', 'satna', 'ratlam', 'rewa', 'katni', 'morwa', 'vindhyanagar', 'jayant', 'dudhichua'
  ],
  'odisha': [
    'bhubaneswar', 'cuttack', 'rourkela', 'brahmapur', 'berhampur', 'sambalpur', 'puri', 
    'balasore', 'bhadrak', 'baripada', 'jharsuguda', 'jeypore', 'rayagada', 'angul', 'balangir'
  ],
  'uttar-pradesh': [
    'lucknow', 'kanpur', 'ghaziabad', 'agra', 'meerut', 'varanasi', 'prayagraj', 'allahabad', 
    'bareilly', 'aligarh', 'moradabad', 'saharanpur', 'gorakhpur', 'noida', 'greater-noida', 
    'jhansi', 'muzaffarnagar', 'mathura', 'ayodhya', 'faizabad', 'firozabad', 'mirzapur', 
    'jaunpur', 'hapur', 'loni', 'pilkhuwa', 'coming-soon'
  ]
};
